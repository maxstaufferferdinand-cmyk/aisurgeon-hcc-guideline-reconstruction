...
DEFAULT_HCC_ROOT = Path(os.environ.get("HCC_ROOT", "."))
DEFAULT_SOURCE_PDF = Path(
    os.environ.get(
        "HCC_SOURCE_PDF",
        os.environ.get("HCC_PDF", str(DEFAULT_HCC_ROOT / "input" / "ESMOHCC2012.pdf")),
    )
)
...
def main() -> int:
    ...
    parser.add_argument(
        "--offline",
        action="store_true",
        help="Skip provider model inventory calls for no-network validation.",
    )
